...
const emptyForm = (): FormData => ({
    description: '', amount: '', category: 'project_payment', source: '', date: new Date().toISOString().split('T')[0],
    projectId: '', items: [],
});
...
const Incomes = () => {
    ...
    /** One income, fully documented: what it was, what it covered, who recorded it —
     * a self-contained voucher rather than a row in a bulk spreadsheet. */
    const exportIncome = (item: Income) => {
        const doc = new jsPDF();
        const brown = '#1B2042';
        ...
        if (item.reference) details.push(['Reference', item.reference]);
        ...
        if (item.items && item.items.length > 0) {
            ...
            doc.text('What This Covered', 14, y);
            autoTable(doc, {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            showToast(editing ? 'Income updated' : 'Income recorded', 'success');
            setShowModal(false);
            fetch();
        } catch (e: any) {
            const message = e?.response?.data?.message;
            showToast(Array.isArray(message) ? message.join('. ') : (typeof message === 'string' ? message : 'Failed to save income'), 'error');
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className="admin-page">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.4rem', marginBottom: '0.6rem' }}>
                ...
                <StatTile icon={<FaHome />} label="Rental" value={`RWF ${totals.rental.toLocaleString()}`} accent="#e11d48" />
                <StatTile icon={<FaChartLine />} label="Investment" value={`RWF ${totals.investment.toLocaleString()}`} accent="#f59e0b" />
                <StatTile icon={<FaComments />} label="Consulting" value={`RWF ${totals.consulting.toLocaleString()}`} accent="#06b6d4" />
            </div>
            ...
            <div className="admin-card">
                ...
                <div style={{ overflowX: 'auto' }}>
                    <table className="admin-table">
                        <thead>
                            <tr>
                                <th>Description</th><th>Amount</th><th>Category</th><th>Project</th><th>Source</th><th>Date</th><th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {paginated.map(item => (
                            ...
                            <div style={{ marginTop: '1.25rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                                    <label className="form-label" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
                                        <FaListUl size={11} /> What This Covers <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(optional — e.g. materials, labor)</span>
                                    </label>
                                ...
                                {form.items.length === 0 ? (
                                    <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', margin: 0 }}>No breakdown added — the income will just show its total amount.</p>
                                ) : (
