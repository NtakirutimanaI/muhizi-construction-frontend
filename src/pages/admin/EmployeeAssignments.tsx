...
import { FaTasks, FaEdit, FaTrash, FaPlus, FaTimes as FaTimesIcon, FaArrowsAlt, FaChevronLeft, FaChevronRight, FaExchangeAlt } from 'react-icons/fa';
...
const emptyForm: FormData = { employeeId: '', projectId: '', role: 'worker', startDate: '', endDate: '', isActive: true };
...
const EmployeeAssignments = () => {
    ...
    const fetch = async () => {
        try {
            const [assRes, empRes, projRes] = await Promise.all([
    ...
    const filtered = useMemo(() => {
        ...
        return data.filter(d => {
            const name = `${d.employee?.firstName || ''} ${d.employee?.lastName || ''} ${d.project?.name || ''} ${d.role}`.toLowerCase();
        ...
        <div className="admin-page">
            ...
            <div className="admin-card">
                ...
                <div style={{ overflowX: 'auto' }}>
                    <table className="admin-table">
                        <thead>
                            <tr>
                                <th>Employee</th><th>Project</th><th>Site</th><th>Role</th><th>Start Date</th><th>End Date</th><th>Status</th><th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {paginated.map(item => (
            ...
            {showModal && (
                <div className="admin-modal-overlay" onClick={() => setShowModal(false)}>
                    <div className="admin-modal" onClick={e => e.stopPropagation()} style={{ left: modalPos?.x ?? '50%', top: modalPos?.y ?? '50%', transform: modalPos ? 'none' : 'translate(-50%, -50%)', width: 550 }}>
