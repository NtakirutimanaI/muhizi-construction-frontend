    ...
    return (
        <div className="admin-page">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem', gap: '1rem', flexWrap: 'wrap' }}>
                <h2 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', margin: 0, fontSize: '1rem' }}><FaEnvelope style={{ color: '#1B2042' }} /> Subscribers</h2>
                    ...
                    {selectedIds.size > 0 && (
                        <>
                            <button onClick={handleBulkDeactivate}
                                style={{ padding: '0.15rem 0.4rem', borderRadius: '6px', border: '1px solid #f59e0b', background: 'transparent', color: '#f59e0b', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                                <FaBan size={10} /> Deactivate ({selectedIds.size})
                            </button>
                            <button onClick={handleBulkDelete}
                                style={{ padding: '0.15rem 0.4rem', borderRadius: '6px', border: '1px solid #ef4444', background: 'transparent', color: '#ef4444', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
            ...
            <div className="admin-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                        <input type="text" className="form-input" placeholder="Search email..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} style={{ padding: '0.25rem 0.4rem', fontSize: '0.75rem', width: 250 }} />
                        {(['all', 'active', 'inactive'] as const).map(f => (
                            <button key={f} onClick={() => { setFilter(f); setPage(1); }}
                                style={{ padding: '0.15rem 0.4rem', borderRadius: '12px', border: '1px solid #ddd', background: filter === f ? '#1B2042' : 'transparent', color: filter === f ? '#fff' : '#333', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 600, textTransform: 'capitalize' }}>
                                {f}
                            </button>
                        ))}
                    </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem', padding: '0.3rem 0', flexWrap: 'wrap', gap: 8 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            <span style={{ fontSize: '0.68rem', color: 'gray' }}>Per page:</span>
                            <select className="form-select" style={{ width: 'auto', padding: '0.15rem 1rem 0.15rem 0.35rem', fontSize: '0.68rem' }} value={pageSize} onChange={e => { setPage(1); setPageSize(Number(e.target.value)); }}>
                        ...
                        {pageSize > 0 && totalPages > 1 && (
                            <div style={{ display: 'flex', gap: 2 }}>
                                <button className="admin-btn admin-btn--secondary" style={{ padding: '0.15rem 0.35rem', fontSize: '0.65rem' }} disabled={page <= 1} onClick={() => setPage(p => Math.max(1, p - 1))}><FaChevronLeft size={10} /></button>
                                {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                                    <button key={p} className={p === page ? 'admin-btn' : 'admin-btn admin-btn--secondary'} style={{ padding: '0.15rem 0.4rem', minWidth: 22, fontSize: '0.68rem' }} onClick={() => setPage(p)}>{p}</button>
                                ))}
                                <button className="admin-btn admin-btn--secondary" style={{ padding: '0.15rem 0.35rem', fontSize: '0.65rem' }} disabled={page >= totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))}><FaChevronRight size={10} /></button>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {showSendUpdate && (
                <div className="admin-modal-overlay" onClick={() => !sending && setShowSendUpdate(false)}>
                    <div className="admin-modal" onClick={e => e.stopPropagation()}>
                        <div className="admin-modal-header">
                            ...
                            <button onClick={() => setShowSendUpdate(false)} disabled={sending}><FaTimes /></button>
                        </div>
