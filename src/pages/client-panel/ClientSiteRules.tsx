...
const ClientSiteRules = () => {
    const [rules, setRules] = useState<SiteRule[]>([]);
    const [loading, setLoading] = useState(false);
    const [selectedRule, setSelectedRule] = useState<SiteRule | null>(null);

    useEffect(() => {
        const cached = loadPageCache<SiteRule[]>('pg_client_site_rules');
        if (cached) setRules(cached);

        setLoading(true);
        siteRulesService.getAll()
            .then(res => {
                setRules(res.data || []);
                savePageCache('pg_client_site_rules', res.data || []);
            })
            .catch(() => {})
            ...
            <div style={{
                ...
                <div style={{ position: 'relative', zIndex: 1, marginBottom: '1.5rem' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.75rem' }}>
                        {[
                            ...
                            { value: 'Active', label: 'Board Status', icon: <FaCheckCircle size={14} /> },
                        ].map((stat, i) => (
                            <div key={i} style={{
                ...
                {loading && rules.length === 0 ? (
                    ...
                    <div style={{
                        ...
                        {rules.filter(r => r.isActive).map((section, idx) => (
                            <div key={section.id}>
                                <div
                ...
                <div style={{
                    ...
                    <span style={{ fontSize: '0.8rem', opacity: 0.9 }}>
                        <strong>Questions or concerns?</strong> Contact your project manager or email{' '}
                        <strong>info@muhiziconstruction.rw</strong>
                    </span>
                </div>
...
export default ClientSiteRules;
